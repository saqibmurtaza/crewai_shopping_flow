...
#         # If there are no recommendations, treat the message as a search query.
...
#             print(f"DEBUG: INTERACTION SEARCH: {self.state.search_results}")
            
#             results = self.state.search_results
#             output = "🔍 **Search Results & Recommendations:**\n\n"
#             for prod in results:
#                 # Normalize keys to remove extra spaces
#                 prod = {k.strip(): v for k, v in prod.items()}
#                 output += (
#                     f"- **{prod['name']}**\n"
#                     f"  - 💰 Price: **${prod['price']}**\n"
#                     f"  - 🏷️ Category: {prod['category']}\n"
#                     f"  - ⭐ Rating: {prod['rating']}\n"
#                     f"  - 📦 Stock: {prod['stock']}\n\n"
#                 )
            
#             self.state.recommendations = results
...
#             # Optionally, you can reset the user_query if needed or handle additional logic.
#             return

#         # If recommendations already exist, process the message as an action command.
...
#         else:
#             await cl.Message(content="No matching products found. Please refine your search.").send()
#             return
...
#     #                 # <<<< Minimal modification: merge complementary products into the existing recommendations
#     #                 # Use self.state.recommendations (which already has the search results) as the base.
#     #                 all_products = {prod.get("name"): prod for prod in self.state.recommendations}
...
class ShoppingFlow(Flow[ShoppingState]):
    ...
    #     user_action = message.content.strip()
        
    #     # Check if we have any recommendations
    #     has_recommendations = bool(self.state.search_results)
    #     print(f"DEBUG: Has recommendations: {has_recommendations}")
    #     print(f"DEBUG: Recommendations: {self.state.recommendations}")

    #     # If we already have search results, process the message as a command
    ...
    #         elif user_action.startswith("update") or user_action.startswith("remove") or user_action == "clear cart":
    ...
    #     else:
    #         # No recommendations yet, treat as initial search query
    #         self.state.user_query = user_action
    #         print(f"DEBUG: User query: {self.state.user_query}")
    #         await cl.Message(content=f"Searching for products matching '{self.state.user_query}'...").send()
    #         await self.search_products()
    ...
    #             for prod in self.state.search_results:
    #                 prod = {k.strip(): v for k, v in prod.items()}
    #                 rec_str += f"- {prod.get('name')} | Price: ${prod.get('price')} | category: ${prod.get('category')}\n"
    #             await cl.Message(content=rec_str).send()
    #             prompt = (
    #                 "What would you like to do next?\n"
    #                 "Type 'refine <query>' to refine your search,\n"
    ...
    async def interaction_agent(self, message: cl.Message):
        """
        The interaction agent handles incoming user messages.
        If no search results exist, the message is treated as a search query.
        Otherwise, the message is processed as an action command.
        """
        # Get the user's message
        user_action = message.content.strip()
        ...
        has_search_results = bool(self.state.search_results)
        print(f"DEBUG: Has search results: {has_search_results}")
        ...
        if has_search_results:
            ...
            # No search results yet, treat as initial search query
            self.state.user_query = user_action
            print(f"DEBUG: User query: {self.state.user_query}")
            await cl.Message(content=f"Searching for products matching '{self.state.user_query}'...").send()
            await self.search_products()
            ...
            if self.state.search_results:
                ...
                for prod in self.state.search_results:
                    prod = {k.strip(): v for k, v in prod.items()}
                    rec_str += f"- {prod.get('name', )} | Price: ${prod.get('price', 'N/A')}\n"
                await cl.Message(content=rec_str).send()
                prompt = (
                    "What would you like to do next?\n"
                    "Type 'refine <query>' to refine your search,\n"
